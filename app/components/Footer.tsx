  ...
  return (
    <footer className="pt-16 pb-8" style={{ background:"var(--dark)", borderTop:"1px solid rgba(232,86,10,.2)" }}>
      <div className="max-w-[1200px] mx-auto px-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 mb-10">
          <div>
            <a href="#home" className="flex items-center gap-3 no-underline mb-4 group">
              <div className="w-10 h-10 flex items-center justify-center transition-all duration-300 group-hover:shadow-[0_0_20px_rgba(232,86,10,.7)]"
            ...
            <p className="text-[var(--text)] text-[0.85rem] leading-relaxed mb-5">
              Premium welding and metalwork solutions for homes and businesses across Nairobi. Crafted with precision, built to last.
            </p>
            <div className="flex gap-3">
              {["f","W","ig","☎"].map((icon, i) => (
                <motion.a key={i} href="#" whileHover={{ y:-3 }}
                  className="w-9 h-9 flex items-center justify-center text-[0.8rem] no-underline transition-all duration-300 hover:bg-[var(--orange)] hover:border-[var(--orange)] hover:text-white"
          ...
          <div>
            ...
            <ul className="list-none flex flex-col gap-2">
              {["#products","#services","#customization","#gallery","#contact"].map(href => (
                <li key={href}>
                  <a href={href} className="text-[var(--text)] text-[0.85rem] no-underline hover:text-white hover:pl-2 transition-all duration-200" style={{ display:"inline-block" }}>
          ...
          <div>
            ...
            <div className="flex flex-col gap-3 text-[0.85rem]" style={{ color:"var(--text)" }}>
              {[
                { icon:"📞", text:"0722 909 059" },
                { icon:"💬", text:"0722 909 059 (WhatsApp)" },
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center pt-6 gap-2 text-[0.78rem]"
          style={{ borderTop:"1px solid rgba(255,255,255,.06)", color:"rgba(176,186,200,.5)" }}>
